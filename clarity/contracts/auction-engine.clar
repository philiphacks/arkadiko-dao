...
(define-data-var auction-ids (list 10000 uint) (list u0))
...
(define-public (start-auction (ustx-amount uint))
  (let ((auction-id (+ (var-get last-auction-id) u1)))
    (map-set auctions
      { id: auction-id }
      { id: auction-id, ustx-amount: ustx-amount, is-open: true }
    )
    (var-set auction-ids (unwrap-panic (as-max-len? (append (var-get auction-ids) auction-id) u10000)))
    (ok true)
  )
)
