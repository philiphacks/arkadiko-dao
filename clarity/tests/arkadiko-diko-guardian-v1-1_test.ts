...
async fn(chain: Chain, accounts: Map<string, Account>) {
  ...
  // Get rewards at start
  let call = chain.callReadOnlyFn("arkadiko-diko-guardian-v1-1", "get-pending-founders-tokens", [], wallet_1.address);
  ...
  // Get rewards
  call = chain.callReadOnlyFn("arkadiko-diko-guardian-v1-1", "get-pending-founders-tokens", [], wallet_1.address);
  call.result.expectOk().expectUint(0)
...
async fn(chain: Chain, accounts: Map<string, Account>) {
  ...
  let block = chain.mineBlock([
    Tx.contractCall("arkadiko-diko-guardian-v1-1", "founders-claim-tokens", [
  ...
  call = chain.callReadOnlyFn("arkadiko-diko-guardian-v1-1", "get-claimed-founders-tokens", [], deployer.address);
  call.result.expectUint(5250000000000);

  // Get rewards at start
  call = chain.callReadOnlyFn("arkadiko-diko-guardian-v1-1", "get-pending-founders-tokens", [], deployer.address);
  call.result.expectOk().expectUint(0)
...
async fn(chain: Chain, accounts: Map<string, Account>) {
  ...
  block = chain.mineBlock([
    Tx.contractCall("arkadiko-diko-guardian-v1-1", "set-founders-wallet", [
